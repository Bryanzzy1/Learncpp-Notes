...
Each chapter starts as a raw export from Notion (a `.zip` file). The conversion is handled by a [Claude Code](https://claude.ai/code) skill — drop a new chapter zip into `learncpp/`, then run `/new-learncpp-chapter`. The skill extracts the Notion export, splits the flat note into individual concept notes with proper frontmatter, decides a node/subnode hierarchy, wires cross-chapter wikilinks to existing notes, and deletes the original flat file.

The skill file lives at `.claude/commands/new-learncpp-chapter.md` in this vault. Its full source is below.

---

## Claude skill — `new-learncpp-chapter`
